...
SinglyLinkedListNode* insert_node_at_tail(SinglyLinkedListNode* head, int node_data)
{
  SinglyLinkedListNode* node = malloc(sizeof(SinglyLinkedListNode));
  SinglyLinkedListNode* ptr_iterator = head;
  ...
  {
    while (ptr_iterator->next)
      ptr_iterator = ptr_iterator->next;
    
    ptr_iterator->next = node;
  }
...
int main()
{
  ...
  for (int i = 0; i < llist_count; i++)
  {
    ...
    SinglyLinkedListNode* llist_head = insert_node_at_tail(llist->head, llist_item);
...
char* readline()
{
  ...
  char* data =(char *) malloc(sizeof(char) * alloc_length);
